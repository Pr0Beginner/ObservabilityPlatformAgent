...
def test_request_allows_future_fields_but_requires_utc() -> None:
    data = {
        "eventId": "evt-1", "taskId": "task-1", "incidentId": "incident-1",
        "version": 1, "requestedAt": "2026-09-16T10:00:00Z", "futureField": True,
    }
...
def test_completed_event_matches_java_field_contract() -> None:
    event = DiagnosisCompletedEvent(
        event_id="evt", task_id="task", incident_id="incident", version=1,
        root_cause="root", confidence=0.8, evidence=[], recommendations=[], tool_calls=[],
        completed_at=datetime.now(UTC), error=None,
    )
    payload = json.loads(event.model_dump_json(by_alias=True, exclude_none=False))
    assert set(payload) == {
        "eventId", "taskId", "incidentId", "version", "rootCause", "confidence",
        "evidence", "recommendations", "toolCalls", "completedAt", "error",
    }
